...
#
# The guard evaluates the *command being invoked*, not quoted data passed to it.
# Quoted spans are stripped before matching, so a command whose argument merely
# quotes a banned word — a commit message, an echo/grep string — is not blocked.
# (A banned word inside quotes is only an issue for adversarial evasion, which
# this accidental-mistake guardrail is not trying to stop.)
#
# Contract:
...
# Collapse newlines first (a quoted span — e.g. a multi-line commit message —
# can cross lines), then strip double- and single-quoted spans so only unquoted
# command tokens remain.
...
if printf '%s' "$SCAN" | grep -qE '(^|[[:space:];&|(])(npm|yarn)([[:space:]]|$)'; then
...
if printf '%s' "$SCAN" | grep -qE '(^|[[:space:];&|(])(eslint|prettier)([[:space:]]|$)'; then
...
# invocation that also references a *.sqlite file. Drop specific tables instead.
if printf '%s' "$SCAN" | grep -qE '(^|[[:space:];&|(])rm([[:space:]]|$)' \
  && printf '%s' "$SCAN" | grep -qiE '\.sqlite'; then
...
if printf '%s' "$SCAN" | grep -qE 'payload[[:space:]]+migrate($|[^:])'; then
  if ! printf '%s' "$SCAN" | grep -qE 'REMOTE_BINDINGS=true'; then
...
if printf '%s' "$SCAN" | grep -qiE 'CONFIRM_REMOTE=yes'; then
