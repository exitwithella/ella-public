...
function InlineCallout({
  ...
  const bgColor = isPressure
    ? 'bg-moss-800/90 border-moss-600/50'
    : 'bg-ash-800/90 border-goldenrod-600/50'
  const labelColor = isPressure ? 'text-moss-300' : 'text-goldenrod-400'
  const arrowColor = isPressure ? 'text-moss-400' : 'text-goldenrod-500'
  ...
  return (
    <motion.div
      ...
      className={`text-ash-50 flex items-center gap-2 rounded-md border px-3 py-2 text-sm md:hidden ${bgColor} ${
        isPressure ? 'self-start' : 'self-end'
      }`}
    >
      {isPressure && <span className={`${arrowColor} text-xs`}>{'\u2192'}</span>}
      <span className={`${labelColor} text-[10px] font-semibold tracking-wider uppercase`}>
        {type === 'pressure' ? 'Pressure' : 'Erosion'}
      </span>
      <span className="text-ash-50/90">{item}</span>
      {!isPressure && <span className={`${arrowColor} text-xs`}>{'\u2190'}</span>}
    </motion.div>
  )
}
...
function QuoteBlock({
  ...
  return (
    <motion.blockquote
      ...
      className="border-moss-500/50 relative z-20 origin-center border-l-2 py-4 pl-6"
    >
      <p className="text-ash-800 font-serif text-lg leading-relaxed italic md:text-xl">{quote}</p>
      <footer className="text-ash-600 mt-3 text-sm tracking-wide uppercase">{attribution}</footer>
    </motion.blockquote>
  )
}
      ...
      {contentItems.map((item, i) => {
        ...
        return (
          <div key={i} className="contents">
            ...
            {item.type === 'paragraph' && (
              <ContentBlock index={i} step={step} onStepChange={handleStepChange} isMobile={isMobile}>
            ...
            {item.type === 'closer' && (
              <ContentBlock index={i} step={step} onStepChange={handleStepChange} isMobile={isMobile}>
                <p className="text-foreground text-xl leading-relaxed font-medium text-balance md:text-2xl">
                  {item.text}
                </p>
              </ContentBlock>
            )}
