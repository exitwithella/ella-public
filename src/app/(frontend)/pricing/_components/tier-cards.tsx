...
function formatPrice(tier: PricingTier): { display: string; period: string | null } {
  ...
  return (
    <div
      className={`relative flex flex-col rounded-sm p-8 ${
        isHighlighted ? 'border-moss-300 bg-moss-50 border-2' : 'border-ash-200 bg-ash-50 border'
      }`}
    >
      {/* Badge */}
      {tier.badge && (
      ...
      {/* Tagline */}
      {tier.tagline && <p className="text-ash-500 mb-6 text-sm">{tier.tagline}</p>}
      ...
      <div className="mb-6">
        {isContactUs ? (
          ...
          <div className="flex items-baseline gap-1">
            ...
            {pricePeriod && <span className="text-ash-500 text-base">{pricePeriod}</span>}
          </div>
        )}
      </div>
      ...
      <a
        ...
        className={`mt-auto inline-flex w-full items-center justify-center gap-2 rounded-full px-6 py-3 text-sm font-semibold transition-colors ${
          isHighlighted
            ? 'bg-moss-700 text-ash-50 hover:bg-moss-800'
    ...
    <section className="bg-ash-100 py-16 md:py-20">
      <Container>
        <div className="mx-auto max-w-4xl">
          ...
          <p className="text-ash-500 mt-6 text-center text-sm">
