    ...
    >
      <div className="bg-ash-50 border-ash-200 w-72 rounded-lg border p-2 shadow-lg shadow-ash-900/8">
        {items.map((item) => (
          ...
          >
            <span className="text-ash-900 text-sm font-medium group-hover:text-moss-700">
              {item.label}
            </span>
            {item.description && (
...
function MobileNavItem({
  ...
  if (link.type === 'link') {
    return (
      ...
      >
        {link.label}
        <span
          className="inline-flex p-1.5 opacity-0 group-hover:opacity-100"
          aria-hidden="true"
        >
  ...
  return (
    <div>
      ...
      {expanded && (
        <div className="mt-3 ml-1 flex flex-col gap-3 border-l-2 border-moss-200 pl-5">
