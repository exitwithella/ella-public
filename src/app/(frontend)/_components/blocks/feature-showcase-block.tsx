...
function headingClasses(block: {
  headingFont?: string | null;
  headingSize?: string | null;
}) {
  const font =
    HEADING_FONT_CLASS[block.headingFont ?? "display"] ??
    HEADING_FONT_CLASS.display;
...
function HeaderTextContent({ block }: { block: FeatureShowcaseData }) {
  ...
  return (
    <>
      ...
      {block.heading && (
        <Heading className={headingClasses(block)}>{block.heading}</Heading>
      )}
...
function Gallery({ block }: { block: FeatureShowcaseData }) {
  ...
  const startCol =
    alignEnd && itemCount < colCount ? colCount - itemCount + 1 : 0;
  const firstItemStyle =
    startCol > 0 ? ({ gridColumnStart: startCol } as const) : undefined;
      ...
      {block.galleryItems.map((item, index) => {
        ...
        return (
          <GalleryItem
            ...
            anchorTarget={
              (item as { anchorTarget?: string | null }).anchorTarget
            }
            sizes={sizes}
            aspect={aspect}
            sharp={sharp}
      ...
      {hasHeader &&
        (isWideHeader ? (
          <div className="mx-auto w-full max-w-[1680px] px-6">
            ...
            {(layout === "text-left" ||
              layout === "text-left-even" ||
              layout === "image-left") && <SplitHeader block={block} />}
