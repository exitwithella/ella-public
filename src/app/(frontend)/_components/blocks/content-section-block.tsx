  ...
  return (
    <ThemeSection bgStyle={block.bgStyle} className="py-20 md:py-28">
      <Container>
        {/* Badge */}
        {block.badge && (
          <span className="bg-theme-surface text-theme-accent mb-4 inline-block rounded-full px-3 py-1 text-xs font-semibold tracking-wide uppercase">
            {block.badge}
          </span>
        )}
        ...
        >
          ...
          <div className={isTwoColumn ? 'flex-1' : 'max-w-[680px]'}>
            ...
            {block.body && (
              <div
                className={`prose prose-lg text-theme-text-secondary mt-6 max-w-none ${isDarkTheme(block.bgStyle) ? 'prose-invert' : ''}`}
              >
