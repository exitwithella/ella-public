...
export async function TrustSecurityBlock({ block }: TrustSecurityBlockProps) {
  const patternMedia =
    block.patternSvg && typeof block.patternSvg !== 'number' ? block.patternSvg : null
  const patternDataUri = patternMedia ? await fetchSvgDataUri(patternMedia) : null
          ...
          >
            <div className="grid gap-8 lg:grid-cols-[1fr_2fr] lg:gap-10">
              <div>
                ...
                {block.intro && (
                  <div
                    className={`prose prose-sm text-theme-text-secondary mb-6 max-w-none [&_p]:text-sm [&_p]:leading-snug [&_p+p]:mt-2 ${isDarkTheme(block.bgStyle) ? 'prose-invert' : ''}`}
                  >
              ...
              {sections.length > 0 && (
                <div className="grid gap-x-8 gap-y-5 sm:grid-cols-2 md:gap-y-6">
                  {sections.map((item) => (
                    <div key={item.id} className="border-theme-border border-t pt-3">
                      <h3 className="text-theme-text mb-1.5 text-sm font-bold tracking-wider uppercase">
                        {parseEmphasis(item.title)}
                      </h3>
                      {item.body && (
                        <p className="text-theme-text-secondary text-xs/relaxed">{item.body}</p>
                      )}
                    </div>
                  ))}
                </div>
